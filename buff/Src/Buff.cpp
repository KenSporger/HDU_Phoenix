...
bool Detect::getArmorCenter_new( cv::Mat &src, const int bMode,armorData &data ,cv::Point2f offset,const int classiMode) {
    /********************************* 二值化 ************************************/
    Mat binary;
   ...
   // if (sParam.debug)
    ...
    // drawContours(src,armorContours, -1, Scalar(0, 0, 255), 2);
    ...
    if (classiMode==mySonNum) {
        for (size_t i = 0; i < armorContours_size; ++i)// 遍历每个轮廓
        {
            ...
            if (contour_area < 500) continue; // 如果面积太小了，那就直接不管了
